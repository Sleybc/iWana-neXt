...
// 2. completedSections
newParts.push('  const completedSections = SECTIONS.filter((s) => (sectionCompletionById[s.id] ?? 0) >= 100).length;');
newParts.push('');
...
newParts.push('        <p className="rounded-xl border border-iwana-primary/15 bg-iwana-primary/5 px-4 py-3 text-sm text-iwana-primary dark:border-iwana-primary-300/20 dark:bg-iwana-primary-400/10 dark:text-iwana-primary-200">');
newParts.push('          {actionMessage}');
newParts.push('        </p>');
newParts.push('      )}');
newParts.push('      {expediente.dataConsentRevoked && (');
newParts.push('        <p className="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 dark:border-red-900/40 dark:bg-red-900/20 dark:text-red-300">');
newParts.push('          El consentimiento de tratamiento de datos fue revocado.');
newParts.push('        </p>');
newParts.push('      )}');
newParts.push('      {/* Progreso general */}');
newParts.push('      <div className="rounded-2xl border border-gray-100 bg-gray-50 p-5 dark:border-dark-border dark:bg-dark-surface-3">');
newParts.push('        <div className="mb-3 flex items-center justify-between text-sm text-gray-600 dark:text-gray-300">');
newParts.push('          <span className="font-medium">Completitud general</span>');
newParts.push('          <span className="font-semibold text-gray-900 dark:text-white">{overallProgress}%</span>');
newParts.push('        </div>');
newParts.push('        <progress');
newParts.push('          value={overallProgress}');
newParts.push('          max={100}');
newParts.push('          className="h-2 w-full overflow-hidden rounded-full [&::-webkit-progress-bar]:bg-gray-200 [&::-webkit-progress-value]:bg-iwana-primary dark:[&::-webkit-progress-bar]:bg-dark-surface-4 dark:[&::-webkit-progress-value]:bg-iwana-secondary [&::-moz-progress-bar]:bg-iwana-primary dark:[&::-moz-progress-bar]:bg-iwana-secondary"');
newParts.push('        />');
newParts.push('      </div>');
...
['commercial', 'technical', 'legal', 'operational'].forEach((dim) => {
  const labels = { commercial: 'Comercial', technical: 'Tecnica', legal: 'Legal', operational: 'Operativa' };
  const label = labels[dim];
  const pctExpr = 'completenessSnapshot.' + dim;
  newParts.push('        <div className="rounded-2xl border border-gray-100 bg-white p-4 dark:border-dark-border dark:bg-dark-surface-2">');
  newParts.push('          <div className="mb-3 flex items-center justify-between">');
  newParts.push('            <p className="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">' + label + '</p>');
  newParts.push('            <Badge');
  newParts.push('              variant={');
  newParts.push('                ' + pctExpr + ' >= 80 ? \'success\' : ' + pctExpr + ' >= 40 ? \'warning\' : \'neutral\'');
  newParts.push('              }');
  newParts.push('            >');
  ...
  newParts.push('                <li key={sId} className="flex items-center justify-between gap-2">');
  newParts.push('                  <span className="truncate">{sec.label}</span>');
  newParts.push("                  <span className={pct >= 100 ? 'font-semibold text-emerald-600 dark:text-emerald-400' : 'text-gray-400'}>");
  newParts.push('                    {pct}%');
  newParts.push('                  </span>');
  newParts.push('                </li>');
  newParts.push('              ) : null;');
...
newParts.push('      <div className="grid gap-4 rounded-2xl border border-gray-100 bg-gray-50 p-4 dark:border-dark-border dark:bg-dark-surface-3 md:grid-cols-2 xl:grid-cols-4">');
newParts.push('        <div>');
newParts.push('          <p className="text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">Estado actual</p>');
...
newParts.push('        </div>');
newParts.push('        <div>');
newParts.push("          <p className=\"text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400\">Fuente</p>");
...
newParts.push('        </div>');
newParts.push('        <div>');
newParts.push("          <p className=\"text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400\">Municipio</p>");
newParts.push('          <p className="mt-1 text-sm font-semibold text-gray-900 dark:text-white">');
newParts.push("            {expediente.municipality || 'Sin municipio'}");
newParts.push('          </p>');
newParts.push('        </div>');
newParts.push('        <div>');
newParts.push("          <p className=\"text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400\">Responsable</p>");
newParts.push('          <p className="mt-1 text-sm font-semibold text-gray-900 dark:text-white">');
...
newParts.push('      <div className="rounded-2xl border border-gray-100 bg-white p-5 dark:border-dark-border dark:bg-dark-surface-2">');
newParts.push("        <p className=\"mb-4 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400\">");
newParts.push('          Acciones de pipeline');
newParts.push('        </p>');
...
newParts.push('      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-gray-100 bg-gray-50 px-5 py-4 dark:border-dark-border dark:bg-dark-surface-3">');
...
newParts.push('      <div className="overflow-hidden rounded-2xl border border-gray-100 dark:border-dark-border">');
...
newParts.push("        subtitle={`Oportunidad ${expediente.id.slice(0, 8).toUpperCase()} \u00b7 Gesti\u00f3n progresiva comercial y operativa.`}");
newParts.push('      />');
