 ...
 */
function ProgressMeter({ value, dimensions, className }: ProgressMeterProps) {
  const pct = Math.max(0, Math.min(100, value));

  return (
    <div className={cn('space-y-4', className)}>
      <div className="mb-1 flex items-end justify-between gap-4">
        <span className="text-sm font-semibold text-iwana-primary dark:text-white/90">
          Informacion general
        </span>
      ...
      <progress
        ...
        aria-label={`Informacion general: ${pct}%`}
        value={pct}
        max={100}
      />
          ...
          {dimensions.map((dim) => (
            ...
            >
              {dim.value === 100 ? (
                ...
                <svg className="h-3.5 w-3.5 shrink-0" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" aria-hidden="true">
