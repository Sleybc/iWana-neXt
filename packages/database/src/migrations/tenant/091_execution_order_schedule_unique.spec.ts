...
describe('ExecutionOrderScheduleUnique091', () => {
  it('es reversible y no fija un schema tenant', async () => {
    ...
    expect(query.mock.calls[0][0]).toContain('UNIQUE INDEX');
    expect(query.mock.calls[0][0]).toContain('tenant_id, schedule_event_id');
    expect(query.mock.calls[1][0]).toContain('DROP INDEX');
