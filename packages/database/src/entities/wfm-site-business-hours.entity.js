'use strict';
var __decorate =
  (this && this.__decorate) ||
  function (decorators, target, key, desc) {
    var c = arguments.length,
      r =
        c < 3
          ? target
          : desc === null
            ? (desc = Object.getOwnPropertyDescriptor(target, key))
            : desc,
      d;
    if (typeof Reflect === 'object' && typeof Reflect.decorate === 'function')
      r = Reflect.decorate(decorators, target, key, desc);
    else
      for (var i = decorators.length - 1; i >= 0; i--)
        if ((d = decorators[i]))
          r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return (c > 3 && r && Object.defineProperty(target, key, r), r);
  };
var __metadata =
  (this && this.__metadata) ||
  function (k, v) {
    if (typeof Reflect === 'object' && typeof Reflect.metadata === 'function')
      return Reflect.metadata(k, v);
  };
Object.defineProperty(exports, '__esModule', { value: true });
exports.WfmSiteBusinessHours = void 0;
const typeorm_1 = require('typeorm');
const shared_1 = require('@iwana/shared');
let WfmSiteBusinessHours = class WfmSiteBusinessHours {
  id;
  tenantId;
  organizationSiteId;
  weekday;
  startTime;
  endTime;
  isEnabled;
  createdAt;
  updatedAt;
};
exports.WfmSiteBusinessHours = WfmSiteBusinessHours;
__decorate(
  [(0, typeorm_1.PrimaryGeneratedColumn)('uuid'), __metadata('design:type', String)],
  WfmSiteBusinessHours.prototype,
  'id',
  void 0,
);
__decorate(
  [(0, typeorm_1.Column)({ name: 'tenant_id', type: 'uuid' }), __metadata('design:type', String)],
  WfmSiteBusinessHours.prototype,
  'tenantId',
  void 0,
);
__decorate(
  [
    (0, typeorm_1.Column)({ name: 'organization_site_id', type: 'uuid' }),
    __metadata('design:type', String),
  ],
  WfmSiteBusinessHours.prototype,
  'organizationSiteId',
  void 0,
);
__decorate(
  [
    (0, typeorm_1.Column)({
      type: 'enum',
      enum: shared_1.BusinessHoursWeekday,
      enumName: 'business_hours_weekday_enum',
    }),
    __metadata('design:type', String),
  ],
  WfmSiteBusinessHours.prototype,
  'weekday',
  void 0,
);
__decorate(
  [
    (0, typeorm_1.Column)({ name: 'start_time', type: 'time', nullable: true }),
    __metadata('design:type', Object),
  ],
  WfmSiteBusinessHours.prototype,
  'startTime',
  void 0,
);
__decorate(
  [
    (0, typeorm_1.Column)({ name: 'end_time', type: 'time', nullable: true }),
    __metadata('design:type', Object),
  ],
  WfmSiteBusinessHours.prototype,
  'endTime',
  void 0,
);
__decorate(
  [
    (0, typeorm_1.Column)({ name: 'is_enabled', type: 'boolean', default: true }),
    __metadata('design:type', Boolean),
  ],
  WfmSiteBusinessHours.prototype,
  'isEnabled',
  void 0,
);
__decorate(
  [
    (0, typeorm_1.CreateDateColumn)({ name: 'created_at', type: 'timestamptz' }),
    __metadata('design:type', Date),
  ],
  WfmSiteBusinessHours.prototype,
  'createdAt',
  void 0,
);
__decorate(
  [
    (0, typeorm_1.UpdateDateColumn)({ name: 'updated_at', type: 'timestamptz' }),
    __metadata('design:type', Date),
  ],
  WfmSiteBusinessHours.prototype,
  'updatedAt',
  void 0,
);
exports.WfmSiteBusinessHours = WfmSiteBusinessHours = __decorate(
  [
    (0, typeorm_1.Index)(
      'uq_wfm_site_business_hours_org_site_weekday',
      ['tenantId', 'organizationSiteId', 'weekday'],
      {
        unique: true,
      },
    ),
    (0, typeorm_1.Index)('idx_wfm_site_business_hours_tenant_org_site', [
      'tenantId',
      'organizationSiteId',
    ]),
    (0, typeorm_1.Entity)({ name: 'wfm_site_business_hours' }),
  ],
  WfmSiteBusinessHours,
);
//# sourceMappingURL=wfm-site-business-hours.entity.js.map
