...
- Nunca usar `synchronize: true`; todos los cambios estructurales deben ir por migraciones reversibles y revisables.
...
- Con pgBouncer en transaction pooling, usar `SET LOCAL search_path` por transaccion; no confiar en `SET` persistente.
...
- No hardcodear schemas ni asumir un tenant unico en scripts o migraciones.
...
- Tras cambios en `packages/database`, actualizar el informe vivo relacionado en `docs/informes/`.
