...
describe('ExpedienteService', () => {
  ...
  it('preserva el error de persistencia aunque falle el rollback físico', async () => {
    ...
    expect(rename).toHaveBeenNthCalledWith(
      1,
      expect.stringMatching(/[/\\]tenant_test[/\\]exp-doc-delete-rollback[/\\]rut[/\\]ver-2\.pdf$/),
      expect.stringMatching(
        /[/\\]tenant_test[/\\]exp-doc-delete-rollback[/\\]rut[/\\]ver-2\.pdf\.pending-delete$/,
      ),
    );
    expect(rename).toHaveBeenNthCalledWith(
      2,
  ...
  it('aplica búsqueda de nombre sin sensibilidad a mayúsculas', async () => {
    const expediente = buildExpediente({
      id: 'exp-search',
      fullName: 'Luis Gomez',
    });
    ...
    await service.findAll({ search: 'luis', page: 1, limit: 10 });

    expect(queryBuilder.andWhere).toHaveBeenCalledWith('expediente.fullName ILIKE :search', {
      search: '%luis%',
    });
  });
  ...
  it('aplica búsqueda por nombre con partición open por defecto y permite all cuando se solicita', async () => {
    const baseRows = [
      buildExpediente({
        id: 'exp-open-luis',
        status: ExpedienteStatus.PRECALIFICADO,
        fullName: 'Luis Prospecto',
      }),
      buildExpediente({
        id: 'exp-active-luis',
    ...
    await expect(service.findAll({ search: 'luis', limit: 20, page: 1 })).resolves.toMatchObject({
      data: [expect.objectContaining({ id: 'exp-open-luis' })],
      total: 1,
    });

    await expect(
      service.findAll({ search: 'luis', view: 'all', limit: 20, page: 1 }),
    ).resolves.toMatchObject({
      data: expect.arrayContaining([
        expect.objectContaining({ id: 'exp-open-luis' }),
        expect.objectContaining({ id: 'exp-active-luis' }),
      ]),
      total: 2,
    });
  });
...
function buildFindAllQueryBuilder(rows: ExpedienteRecord[]) {
  ...
  function applyFilters(list: ExpedienteRecord[]): ExpedienteRecord[] {
    ...
    if (searchFilter) {
      ...
      result = result.filter((r) => r.fullName.toLocaleLowerCase().includes(normalizedSearch));
    }
  ...
  const qb: Record<string, jest.Mock> = {
    andWhere: jest.fn((condition: string, params?: Record<string, unknown>) => {
      if (condition.includes('fullName ILIKE :search') && typeof params?.search === 'string') {
        searchFilter = params.search.replaceAll('%', '');
      }
