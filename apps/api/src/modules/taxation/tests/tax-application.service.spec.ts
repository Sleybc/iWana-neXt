...
describe('TaxApplicationService', () => {
  ...
  describe('resolve — Gobierno con municipalityCode (IVA + RFTE + ICA)', () => {
    it('retorna 3 snapshots: IVA, Retefuente e ICA para el municipio', async () => {
      ...
      const result = await service.resolve({
        ...
        segment: CustomerSegment.GOVERNMENT,
  ...
  describe('ITaxApplicationReadPort token', () => {
    ...
    it('rechaza persona natural sin estrato', async () => {
      await expect(
        service.resolve({
          personType: PersonType.NATURAL,
          segment: CustomerSegment.RESIDENTIAL,
        }),
      ).rejects.toThrow('Estrato es obligatorio para persona natural');
    });
  });
