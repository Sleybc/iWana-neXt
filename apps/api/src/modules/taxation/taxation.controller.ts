...
import { TaxationPaginatedResult } from '../../common/pagination';
...
export class TaxationController {
  ...
  /**
   * Lista definiciones tributarias con filtros y paginación cursor (ADR-064).
  ...
  @ApiOperation({
    summary: 'Listar definiciones tributarias del tenant con filtros',
    description:
      'ADR-064: limit default 20, max 100; meta.nextCursor + meta.total. Orden: code ASC, id ASC. ' +
      'Filtros: category, context, origin, isActive.',
  })
  @ApiResponse({
    status: 200,
  ...
  async findAll(
    @Query() query: ListTaxDefinitionQueryDto,
  ): Promise<TaxationPaginatedResult<TaxDefinition>> {
    return this.taxDefinitionService.findAll(query);
  }
