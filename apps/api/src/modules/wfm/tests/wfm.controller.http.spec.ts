...
describe('WfmController HTTP', () => {
  ...
  const visitRequestsServiceMock = {
    ...
    listEligibleOperationalAssignees: jest.fn(),
  ...
  describe('GET /api/v1/wfm/eligible-assignees', () => {
    it('returns 200 with eligible operational assignees for ADMIN', async () => {
      visitRequestsServiceMock.listEligibleOperationalAssignees.mockResolvedValue([
        {
          id: 'tech-eligible-001',
      ...
      await request(app.getHttpServer())
        .get('/api/v1/wfm/eligible-assignees')
        .set('Authorization', 'Bearer admin-token')
        .expect(200)
        .expect(({ body }) => {
          expect(body).toEqual([
      ...
      expect(visitRequestsServiceMock.listEligibleOperationalAssignees).toHaveBeenCalledWith(
        expect.objectContaining({ role: UserRole.ADMIN }),
      );
    });

    it('returns 200 for SALES because assisted scheduling depends on this source', async () => {
      visitRequestsServiceMock.listEligibleOperationalAssignees.mockResolvedValue([]);
    ...
    it('returns 200 for TECHNICIAN because agenda viewers need the operational directory', async () => {
      visitRequestsServiceMock.listEligibleOperationalAssignees.mockResolvedValue([]);
  ...
  describe('POST /api/v1/wfm/events/:id/move-to-pending', () => {
    it('returns 403 when TECHNICIAN tries to move an event to pending', async () => {
      await request(app.getHttpServer())
    ...
    it('returns 200 when ADMIN moves an event back to pending without reason', async () => {
      scheduleEventsServiceMock.moveToPending.mockResolvedValue({
        id: 'vr-001',
    ...
    it('returns 400 when the linked event cannot be returned to pending', async () => {
      scheduleEventsServiceMock.moveToPending.mockRejectedValue(
        new BadRequestException(
