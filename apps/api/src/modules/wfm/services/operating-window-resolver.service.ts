...
export interface OperatingWindowResult {
  status: 'OPEN' | 'CLOSED';
  source: 'HOLIDAY_BLACKOUT' | 'SITE_HOURS' | 'COMPANY_HOURS' | 'MISSING_CONFIGURATION';
...
export class OperatingWindowResolverService {
  ...
  async resolveWithManager(
    ...
    if (input.organizationSiteId) {
      const siteHours = await manager.findOne(WfmSiteBusinessHours, {
        where: {
