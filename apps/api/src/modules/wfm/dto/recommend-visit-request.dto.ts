...
export const RecommendVisitRequestSchema = z
  .object({
    durationMinutes: z
      .number()
      .int()
  ...
  .refine(
    (value) => {
      if (!value.windowStartAt || !value.windowEndAt) {
        return true;
      }

      return new Date(value.windowEndAt).getTime() > new Date(value.windowStartAt).getTime();
    },
    {
      path: ['windowEndAt'],
...
export type RecommendVisitRequestInput = z.infer<typeof RecommendVisitRequestSchema>;

export class RecommendVisitRequestDto {
  ...
  @ApiPropertyOptional({ example: '2026-06-01T23:00:00Z' })
  @IsOptional()
  @IsDateString({ strict: false })
  windowEndAt?: string;

  @ApiPropertyOptional({ example: 7, minimum: 1, maximum: 14 })
  @IsOptional()
  @Type(() => Number)
  @IsInt()
  @Min(1)
