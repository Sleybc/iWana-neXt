...
export class TenantProvisioningService {
  ...
  async enqueue(payload: ProvisioningJobPayload): Promise<void> {
    try {
      ...
      // Marcar el tenant como fallo de provisioning para visibilidad en dashboard
   ...
   */
  async retryProvisioning(tenantId: string, schemaName: string, tenantSlug: string): Promise<void> {
    // Limpiar el job fallido previo si existe (evitar duplicados)
    await this.provisioningQueue.remove(`provision-${tenantId}`);

    // Resetear estado a PROVISIONING antes de reencolar
    await this.tenantRepo.update(tenantId, { status: TenantStatus.PROVISIONING });
