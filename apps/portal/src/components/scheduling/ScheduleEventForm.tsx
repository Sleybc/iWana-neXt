...
const scheduleEventFormSchema = z
  .object({
    ...
    assignedUserId: z.string().uuid('Selecciona un responsable válido.'),
...
export function ScheduleEventForm({
  ...
  const operationalTechnicians = useMemo(
    () => filterOperationalTechnicians(technicians),
    [technicians],
  );
  ...
  return (
    <form
      ...
      onSubmit={handleSubmit(async (values) => {
        ...
        if (isScheduleStartInPast(scheduleValues.startAt)) {
          setError('scheduledStartTimeLocal', {
            type: 'validate',
            message: SCHEDULE_PAST_NOT_ALLOWED_MESSAGE,
          });
          return;
        }
      ...
      <section className="space-y-4 rounded-2xl border border-gray-200 bg-white p-4 dark:border-dark-border dark:bg-dark-surface-2">
        <div className="space-y-1">
          <h3 className="text-sm font-semibold text-gray-900 dark:text-white">
            {isManualFlow ? 'Visita manual' : 'Tarea'}
          </h3>
          <p className="text-sm text-gray-500 dark:text-gray-400">
            {isManualFlow
              ? 'Confirma técnico, franja y contexto operativo de la visita excepcional.'
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="space-y-1">
              ...
              <p className="max-w-2xl text-sm text-gray-500 dark:text-gray-400">
                Busca franjas libres y prioriza recursos operativos con tareas cercanas por
                municipio, sector/vereda y coordenadas.
              </p>
            </div>
            <Button
              type="button"
              ...
              onClick={async () => {
                ...
                if (coordinates?.trim() && parsedLatitude === null) {
