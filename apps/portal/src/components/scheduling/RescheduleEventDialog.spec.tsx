...
function buildEvent(overrides?: Partial<Record<string, unknown>>) {
  return {
    ...
    scheduledStartAt: new Date('2026-07-15T08:00').toISOString(),
    scheduledEndAt: new Date('2026-07-15T10:00').toISOString(),
...
describe('RescheduleEventDialog', () => {
  ...
  it('hidrata la nueva franja operativa desde el evento', async () => {
    ...
    expect(screen.getByLabelText('Fecha de visita')).toHaveValue('2026-07-15');
    expect(screen.getByLabelText('Hora de llegada')).toHaveValue('08:00');
    expect(screen.getByDisplayValue('2026-07-15 10:00')).toBeInTheDocument();
  });
    ...
    render(
      <RescheduleEventDialog
        ...
        event={
          buildEvent({
            scheduledStartAt: new Date('2026-07-15T08:00').toISOString(),
            scheduledEndAt: new Date('2026-07-15T08:10').toISOString(),
          }) as never
        }
        onOpenChange={jest.fn()}
        onSubmit={onSubmit}
  ...
  it('permite ajustar una duración rápida y recalcula el fin', async () => {
    ...
    expect(screen.getByDisplayValue('2026-07-15 10:30')).toBeInTheDocument();
  });
  ...
  it('envía inicio y fin calculados al reagendar', async () => {
    ...
    await waitFor(() => {
      expect(onSubmit).toHaveBeenCalledWith({
        scheduledStartAt: new Date('2026-07-15T09:00').toISOString(),
      ...
      <RescheduleEventDialog
        ...
        event={
          buildEvent({
            ...
            scheduledEndAt: new Date('2026-07-15T10:30').toISOString(),
          }) as never
        }
        onOpenChange={jest.fn()}
        onSubmit={jest.fn().mockResolvedValue(undefined)}
