...
export function isScheduleStartInPast(
  ...
  return startMs < referenceDate.getTime();
}

export function isScheduleDaySlotInPast(
  dayKey: string,
  timeValue: string,
  referenceDate: Date = new Date(),
): boolean {
  const window = buildScheduleWindow(dayKey, timeValue, 15);

  if (!window) {
    return true;
  }

  return isScheduleStartInPast(window.startAt, referenceDate);
}
