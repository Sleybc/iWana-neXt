...
export function InventoryCatalogProductsSkeleton() {
  return (
    <div className="space-y-4" aria-busy="true" aria-label="Cargando productos">
      <PortalSkeletonBlock className="h-5 w-32 rounded-full" />
      <div className="space-y-3 border-b border-gray-100 pb-4 dark:border-dark-border">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {Array.from({ length: 5 }).map((_, index) => (
            <PortalSkeletonBlock key={index} className="h-12 rounded-2xl" />
          ))}
        </div>
      </div>
