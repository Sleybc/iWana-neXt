...
Se adopta una solución **tables-first tenant-aware** dentro de WFM.
...
2. Se crea una entidad nueva de **sede operativa** para representar oficinas o bases de despacho.
...
- **WFM** es owner del calendario operativo, sedes operativas, festivos, overrides por técnico, agenda y recomendaciones.
- **TenantModule** sigue siendo owner de settings corporativos globales como `timezone`, `currency`, `language` y `country`.
- **MOD03 cobertura** mantiene ownership de `CommercialNode` y `CoverageZone`, que no deben usarse como sustituto de sede física.
