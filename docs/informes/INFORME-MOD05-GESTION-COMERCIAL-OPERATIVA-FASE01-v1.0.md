...
- 2026-05-06: Seguimiento de code review para Task 2 en `apps/api/src/modules/crm/expedientes/expediente.service.ts`. Causa raíz: `deleteDocumentSupport()` ejecutaba `unlink(pendingDeleteFilePath)` fuera del flujo de persistencia y propagaba el error aun cuando la versión ya había sido removida del JSONB en BD. Ajuste aplicado: manejo acotado solo alrededor de `unlink()` para degradar la limpieza física a best-effort después de persistir, con warning por logger sin revertir la operación de negocio. Cobertura: nueva regresión en `apps/api/src/modules/crm/expedientes/tests/expediente.service.spec.ts` validando que la eliminación devuelve éxito y mantiene auditoría/completitud aunque falle el cleanup final. Validación: `pnpm --filter @iwana/api test -- src/modules/crm/expedientes/tests/expediente.service.spec.ts src/modules/crm/expedientes/tests/expedientes.controller.spec.ts`.
