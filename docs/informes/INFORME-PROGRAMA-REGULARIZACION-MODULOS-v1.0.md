...
| --- | --- | --- | --- |
| **CSP del API vía `helmet` no alineada** con la CSP de las apps — `helmet()` protege solo las respuestas del API (JSON) y su política no incluye `frame-ancestors 'none'` ni el baseline emitido por las apps en su HTML | Media — pre-existente, no bloqueante | **AI-PLAT-OPS** | Al ejecutarse el paso 2 de ADR-081 (hardening de CSP con nonces), el disparador que la toca directamente; **se adelanta** si antes se habilita el dominio productivo (ADR-070/078). No hay urgencia calendario mientras no exista dominio productivo |
| Hardening de CSP con nonces (`proxy.ts` + renderizado dinámico) | Media — mejora futura habilitada por ADR-081 | AI-FE-PLATFORM | Al ejecutarse el paso 2 de ADR-081 |
