...
| **Condición de retorno** | Cerrar B1 y las tres fases reabiertas → consolidar G6.5. ~~Bloqueado además por el impedimento transversal de §4~~ (**impedimento RESUELTO** el 2026-08-10, ver §4) |
...
## 4. Impedimento transversal — la salida del exceso estuvo bloqueada (**RESUELTO**)

> **Estado vigente (2026-08-10).** El impedimento descrito abajo quedó **resuelto**: la integración continua volvió a operar y las corridas sobre `main` ya son verdes — entre ellas la [31395607153](https://github.com/Sleybc/iWana-neXt/actions/runs/31395607153) del 2026-08-10 sobre `66e9ce41` (E2E R4.1, lint+typecheck+build+unit, citas ADR e imágenes production, con artefacto sanitizado `e2e-r41-summary`). **G6.5 es obtenible desde entonces**, la cota de dos vuelve a poder reducirse cerrando módulos con su evidencia, y la Ola 0 de la secuencia aprobada (§5) — restablecer la facturación — quedó **cumplida**. El relato original se conserva como histórico, porque registra la causa y el comportamiento que SEC-P1 tuvo que asumir mientras duró.

**Ningún módulo podía obtener G6.5 entonces, por una causa ajena a la ingeniería.**

G6.5 exige, por [ADR-069](../adrs/ADR-069-Gates-G6.5-Merge-Readiness.md), una corrida verde de integración continua en Linux identificada por SHA. La integración continua estaba **en rojo por facturación de GitHub Actions** —*"recent account payments have failed or your spending limit needs to be increased"*—, condición **preexistente en la rama principal** y no introducida por ningún cambio. El frente SEC-P1 lo documentó al detalle: su PR se mergeó con los cinco checks en fallo y quedó registrado como **«MERGEADO sin G6.5»**, con la instrucción de reejecutar CI sobre la rama principal al restablecerse la facturación y tratar como regresión cualquier fallo no atribuible a ella.

**Consecuencia directa (histórica):** mientras la facturación estuvo caída, la cota de dos no podía reducirse cerrando módulos, porque el cierre en construcción exige G6.5 y G6.5 exige CI. Era un impedimento administrativo, no técnico, y bloqueaba el camino de salida de los cuatro módulos que exceden la cota. Esa restricción **dejó de aplicar** cuando CI volvió a estar verde ([§7](#7-registro-de-cambios), entrada 2026-08-10).

**Excepción parcial (histórica):** MOD00 y MOD05 podían avanzar hasta la puerta de G6.5 —consolidar evidencia, emitir informe— y quedar a la espera únicamente de la corrida. La instrucción sigue siendo válida como método: ningún módulo debe dejar la consolidación de evidencia para después de la corrida.
...
| --- | --- | --- |
| **0** | ~~Restablecer la facturación de integración continua~~ — **CUMPLIDA** (CI operativa y en verde; ver §4) | Levanta el impedimento transversal de §4 — **precondición de todo cierre** |
...
> **Estado vigente (2026-08-09).** El párrafo anterior describe el momento del hallazgo. El frente **está cerrado por completo**: C-7, C-7b, C-8 y C-10 corregidos y con cierre formal firmado por AI-SEC-ENG; evidencia reproducida en la auditoría de consolidación (ver [§7](#7-registro-de-cambios) y deuda con dueño en §5.3). No hay ningún hallazgo de este frente abierto.

### 5.3 Deuda transversal registrada del frente XSS — con dueño y fecha de revisión

El cierre formal de C-8 y C-10 ([INFORME-TRANSVERSAL §8](../informes/INFORME-TRANSVERSAL-XSS-BUSQUEDA-GLOBAL-v1.0.md)) registró deudas fuera del cierre. Ninguna queda diferida sin destino: todas tienen dueño y una revisión programada.
...
| 2026-08-09 | **Auditoría de consolidación (AI-EM-ARCH).** **Ola 1 verificada, sin escalación al CTO:** OLA1-a (ADR-072) y OLA1-b (ADR-081) firmadas y Aprobadas; frente XSS cerrado con evidencia propia reproducida en la auditoría. **Corrección del relato (§5.2):** el §5.1 del informe transversal reportaba como vigente un estado intermedio de C-8 (*"pendiente de re-verificación de AI-SEC-ENG"*) cuando el cierre formal ya estaba firmado — defecto que la regla de cita verificada (protocolo §7.4, ADR-056 §5) existe para prevenir; corregido al estado final. **Deuda registrada (§5.3):** CSP del API vía `helmet` no alineada, dueño **AI-PLAT-OPS**, revisada automáticamente en la ejecución de OLA1-b (prompt C-1…C-9, paso 2 del plan de ADR-081) o en la habilitación productiva, lo que ocurra primero |
| 2026-08-09 | **Prompt de ejecución de OLA1-b emitido (paso 2 del plan de ADR-081).** [PROMPT-TRANSVERSAL-OLA1-B-SESION-COOKIE-v1.0.md](../prompts/PROMPT-TRANSVERSAL-OLA1-B-SESION-COOKIE-v1.0.md): C-1…C-9 congeladas como criterios de aceptación, cinco tracks (SR-FULL, FE-PLATFORM, PLAT-OPS, SR-QA, re-verificación SEC-ENG) y las dos revisiones de deuda de §5.3 como checklist obligatorio del encargo. El prompt declara **tres correcciones al estado que suponía el ADR**, verificadas sobre el código: el token `password-change` de web no tiene clave propia (vive en la clave de sesión), no existe helper compartido de siembra E2E (cada spec siembra local; 21 del portal + `portal-admin-first-access`), y `.env.example` no declara `NEXT_PUBLIC_API_URL` ni `COOKIE_SECURE`. Ninguna invalida condición alguna; las tres quedan como trabajo explícito del encargo |
| 2026-08-09 | **OLA1-b ejecutada y CERRADA (fase, paso 1 de la migración en dos pasos de ADR-081).** Consolidación de los cinco tracks: CA-1/CA-2/CA-3 en verde (tenant resuelto solo desde token verificado de cookie, guard CSRF `X-Requested-With` con test negativo, C-3 sin excepción), CA-4/CA-5/CA-6/CA-7 en verde (variables de URL del API separadas por app, cookies `__Host-` con path mínimo y `Secure` acoplado a producción, `platform/login` emite refresh cookie, siembra E2E por cookie). **Evidencia con corridas reales `Cached: 0`:** E2E portal 159/159 con servidor fresco, `pnpm test` 9/9 tareas (API 3126 passed), typecheck 8/8, lint 0 errores. **Punto de no retorno alcanzado:** retiro del soporte de lectura de tokens desde `localStorage` en portal y web (tokens solo en memoria; token limitado `mfa-setup` por flujo de login) con C-3 actualizado. **Re-verificación independiente AI-SEC-ENG (track 5): C-1, C-2 y C-3 APROBADOS.** Las dos revisiones de deuda de §5.3 registradas con decisión y fecha: CSP del API vía `helmet` → diferir; hardening de CSP con nonces → diferir al paso 2. Detalle: [INFORME-OLA1B-FASE-SESION-COOKIE-v1.0.md](../informes/INFORME-OLA1B-FASE-SESION-COOKIE-v1.0.md) |
| 2026-08-10 | **OLA1-b — consolidación CI Linux por SHA (G6.5) y corrección de un flake post-cierre.** El merge a `main` (`9bf776c`) disparó la corrida 31338427530, cuyo job E2E R4.1 falló por un **flake dependiente de la hora de ejecución** en el test 5d — solapamiento de ventanas de agenda entre los tests 1a y 5d con CI en UTC nocturno → `400` del `ScheduleConflictService` —, no por defecto de la migración. Corregido con el commit `66e9ce41` (`nowIso` → `anchorScheduleIso`). La corrida [31395607153](https://github.com/Sleybc/iWana-neXt/actions/runs/31395607153) quedó **completamente verde** (E2E R4.1, lint+typecheck+build+unit, citas ADR, imágenes production) con artefacto sanitizado `e2e-r41-summary`. La no conformidad de secuencia (cierre declarado antes de la evidencia CI Linux) y la consolidación G6.5 quedaron registradas en el informe de fase §3.3 |
| 2026-08-10 | **Impedimento transversal de §4 RESUELTO y corrección documental.** La corrida 31395607153 (misma entrada anterior) demuestra que la integración continua volvió a operar y G6.5 es obtenible: §4 se marca como histórico con sus consecuencias operativas actualizadas (la cota de dos vuelve a poder reducirse; la Ola 0 de §5 quedó cumplida) y las condiciones de retorno que dependían del bloqueo (MOD00) se liberan. **Perfil AI-EM-ARCH a v2.4 (2026-08-10):** la edición previa del protocolo v1.3 → v1.5 se regulariza con bump (las menciones vigentes quedan en v1.5; el historial v2.1→v2.2 restaura v1.3, que era la versión de ese momento) y la Parte II sincroniza su `Versión del Perfil` |
