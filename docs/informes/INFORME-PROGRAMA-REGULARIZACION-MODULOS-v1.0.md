...
| --- | --- | --- | --- |
| **CSP del API vía `helmet` no alineada** con la CSP de las apps — `helmet()` protege solo las respuestas del API (JSON) y su política no incluye `frame-ancestors 'none'` ni el baseline emitido por las apps en su HTML | Media — pre-existente, no bloqueante | **AI-PLAT-OPS** | **En la ejecución de OLA1-b** — el prompt de ejecución con C-1…C-9 como criterios de aceptación (paso 2 del plan de [ADR-081](../adrs/ADR-081-Modelo-de-Sesion-Cookie-HttpOnly.md)) — que la revisa automáticamente; se adelanta si antes se habilita el dominio productivo (ADR-070/078). No hay urgencia calendario mientras no exista dominio productivo |
| Hardening de CSP con nonces (`proxy.ts` + renderizado dinámico) | Media — mejora futura habilitada por ADR-081 | AI-FE-PLATFORM | Revisada en la misma ejecución de OLA1-b; su cierre definitivo requiere además `proxy.ts` + renderizado dinámico (paso 2 de la migración en dos pasos de ADR-081) |
