...
**Pendientes fuera de este cierre (con dueño y fecha de revisión, no bloqueantes):** hardening de la CSP con nonces (AI-FE-PLATFORM, habilitado por ADR-081) y CSP del API vía `helmet` no alineada (AI-PLAT-OPS), **revisadas automáticamente en la ejecución de OLA1-b** — prompt de ejecución con C-1…C-9 como criterios de aceptación, paso 2 del plan de [ADR-081](../adrs/ADR-081-Modelo-de-Sesion-Cookie-HttpOnly.md) —; revisión de CSP ante CDN futuro (AI-PLAT-OPS), anclada a cualquier cambio de topología que introduzca CDN. Detalle de fechas en [INFORME-PROGRAMA §5.3](../informes/INFORME-PROGRAMA-REGULARIZACION-MODULOS-v1.0.md). **ADR-081** (sesión cookie `httpOnly`) sigue siendo la capa que impide el robo de token por script.
