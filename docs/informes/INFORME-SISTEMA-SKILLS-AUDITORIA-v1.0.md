...
- Se agregó `references/evaluation-criteria.md` con los criterios detallados por dimensión anclados a artefactos reales del repo (`globals.css`, `portal-ui.tsx`, ADR-056 §2), siguiendo el patrón de progressive disclosure de `skill-creator`.
...
- **Cinco referencias nuevas** (todas <300 líneas, verificadas contra el repo real):
  - `references/tokens.md` — escalas completas con hex exactos de `globals.css`, regla AA del lima (`-700` = `#6A7A1C`, 6.2:1), superficies, dark `dark-surface-*` (ADR-056 §2), radios, sombra dual, y **divergencias conocidas** (`tokens/colors.ts` vs `globals.css`; `--chart-*` aún inexistentes; config vieja del login-prototipo).
