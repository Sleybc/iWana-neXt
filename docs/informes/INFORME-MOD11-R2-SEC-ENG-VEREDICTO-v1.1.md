...
**Acción completada para G6.5:** conservar la evidencia de QA-33 y registrar CI #112 sobre `1343d6b8`, con `production-images` y `execution-orders-e2e` verdes. Security no convierte esa corrida en autorización productiva.

### G6.5 CI — fuera del veredicto de seguridad

El gate está parametrizado con piso **29** y rechazo de flaky > 0 (scripts/e2e-provision-operational.mjs:17-18, :91-105; workflow ci.yml:448-451, :511-529). CI #112 sobre `1343d6b8` confirmó el cumplimiento Linux; **G6.5 queda fuera del veredicto de seguridad** y no autoriza G7.
...
| G6.5-CI | Gate de CI, fuera de seguridad | Cerrado | CI #112 sobre `1343d6b8`: `production-images` y `execution-orders-e2e` verdes; R4.1 29/0/0/0/0, exit 0, flaky=0 y cleanup OK. | AI-PLAT-OPS | Mantener en CI |
...
- **G6.5 CI:** cerrado en CI #112 y explícitamente fuera del veredicto de seguridad.
