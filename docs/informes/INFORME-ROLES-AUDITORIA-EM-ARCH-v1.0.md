...
1. **ADR-069 (propuesto).** No se anticipa la taxonomía G6.5 en §7 ni §11 del perfil mientras el ADR no esté `Aprobado`: hacerlo sería exactamente la infracción del protocolo §7.4 que este informe audita. Al aprobarse, el perfil requiere **v2.3**.
...
4. **`.cursor/` sin versionar** (X-04). O se versiona el adaptador — como `.claude/agents/` y `.opencode/`, que sí lo están y tienen `pnpm sync:agents:check` en CI — o se retira y Cursor se activa por el prompt de `docs/prompts/` como los otros clientes. Mantenerlo ignorado deja una superficie de activación fuera de toda auditoría, que es justo cómo el enlace de X-01 sobrevivió cinco semanas rotos. Recomendación: **versionarlo**, dado que `AGENTS.md` lo trata como superficie de paridad.
