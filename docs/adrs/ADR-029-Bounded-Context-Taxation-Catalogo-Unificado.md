...
> **Actualización ADR-082 (propuesto), 2026-08-20:** las reglas de aplicación a clientes (`tax_rules`, `tax_rule_applications`) pasan a `TaxationModule`. Las reglas de aplicación a proveedores siguen reservadas al futuro `PurchasingModule`. Perfiles fiscales de terceros y el cálculo de factura no cambian de owner (CRM y Billing).

- Reglas de aplicación a clientes: propiedad de `TaxationModule` — ADR-082 (propuesto). Quedó en `CommercialModule` hasta ese ADR.
