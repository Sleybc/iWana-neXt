...
| QA-34 | Ingress efectivo protege TLS | P0 | **QA-34/TLS está diferido por [ADR-070](../adrs/ADR-070-Diferimiento-Dominio-Productivo.md) (superado) (Aprobado, CTO 2026-08-02): el dominio productivo no se define hasta que se cumpla su disparador de reactivación. No bloquea G6 ni G6.5; bloquea G7 hasta que se verifique un certificado de CA reconocida, terminación TLS efectiva y redirección HTTPS sobre el dominio aprobado.** Evidencia G7 requerida y fechada, según `RUNBOOK-RELEASE-ROLLBACK` §2.2 y §8.1–§8.6: dominio aprobado bajo control DNS y certificado de CA reconocida; redirección HTTP→HTTPS; health público por HTTPS; handshake público con hostname y cadena verificados; renovación/ACME `dry-run`, `nginx -t` y reload; rollback al certificado anterior; clave privada solo en secret store o volumen protegido fuera de Git, imágenes, workflows, tickets y logs. G7 sigue **NO-GO** hasta evidenciar todos estos puntos; HSTS queda para una decisión controlada posterior. | [~] DIFERIDO POR ADR-070 — condición de un release no planificado |
...
| QA-36 | Existe E2E vertical con API y PostgreSQL reales | P0 | **R4.1 (2026-08-02):** vertical `execution-orders-operational.spec.ts` **29/29 passed, exit 0, flaky=0**, contra API + PostgreSQL + Redis + MinIO + Typesense + worker BullMQ reales provisionados por `scripts/e2e-provision-operational.mjs`. Defectos 8b, QA-33 ESM y contaminación de bucket BOLA remediados y verificados. CI #112 sobre `1343d6b8` certifica Linux; evidencia sanitizada y artefacto del run archivado. | [x] PASS |
...
- [x] G6.5 Merge readiness: CI #112 certifica `production-images` y `execution-orders-e2e` verdes en Linux sobre `1343d6b8`.
...
**Veredicto actual:** G6 **GO**; G6.5 **GO** tras CI Linux real sobre `1343d6b8`; G7 **NO-GO** para producción. Los seis P0 históricos están remediados y registrados en §7.5. TLS, rollback, restores y aprobación CTO siguen siendo requisitos de G7.
...
| Rate Limit / TLS | QA-33 a QA-34 | 1 | 0 | 0 | **GO** (QA-33 PASS con E2E 429 real; QA-34 diferido por ADR-070 (superado), 2026-08-02) |
...
**Registro histórico de QA-34:** en la foto de 2026-07-31 figuraba **APROBADO CTO — diferido** hasta definición de dominio y no bloqueaba G6/G7. Ese criterio está **superado por §7.5 y la matriz vigente**: QA-34 no bloquea G6/G6.5, pero bloquea G7 hasta evidencia TLS productiva. Desde el 2026-08-02 la autoridad del diferimiento es [ADR-070](../adrs/ADR-070-Diferimiento-Dominio-Productivo.md) (superado), que lo convierte en decisión aprobada con disparador de reactivación en lugar de un pendiente abierto del CTO.
