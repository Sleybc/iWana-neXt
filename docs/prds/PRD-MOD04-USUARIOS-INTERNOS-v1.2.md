...
**Version:** 1.2  
**Estado:** Vigente — política de `documentNumber` corregida por decisión del CTO  
**Fecha:** 2026-09-03  
...
**HLD relacionado:** docs/hlds/HLD-MOD04-USUARIOS-INTERNOS-v1.2.md  
**Informe relacionado:** docs/informes/INFORME-MOD04-PERFIL-PORTAL-AUDITORIA-v1.0.md  
**ADRs aplicables:** ADR-018, ADR-019, ADR-020, ADR-022, ADR-045, ADR-086  
**Version anterior:** docs/prds/PRD-MOD04-USUARIOS-INTERNOS-v1.1.md

---

## Changelog v1.1 → v1.2 (2026-09-03)

Origen: [INFORME-MOD04-PERFIL-PORTAL-AUDITORIA-v1.0](../informes/INFORME-MOD04-PERFIL-PORTAL-AUDITORIA-v1.0.md) §4, hallazgo P-10.

Este PRD afirmaba en **cuatro puntos** que `documentNumber` nunca se incluye en respuestas API. La afirmación era **falsa desde antes de esta revisión** — el código lo devolvía en `findAll` y en `findMe` — y quedó superada por dos decisiones del CTO que el documento nunca recogió:

| Decisión | Fecha | Alcance |
| --- | --- | --- |
| Aceptación de riesgo E-05 | 2026-07-23 | Exposición en el **listado ADMIN** (`findAll`), necesaria para la operación |
| Acceso del titular — [ADR-086](../adrs/ADR-086-Acceso-Titular-Documento-Perfil-Propio.md) (propuesto) | 2026-09-03 | Exposición **y edición en el perfil propio**: el titular verifica y rectifica su documento |

| Sección | Cambio |
| --- | --- |
| §Seguridad (l. 80), §RF-USR-06 (l. 197), §UserResponseDto (l. 427), §Cumplimiento (l. 438) | Las cuatro afirmaciones se marcan **superadas** y se sustituyen por la política vigente |
| §RF-USR-10 (l. 230) | Retirado `mfaRequired` de los campos auto-editables en `PATCH /users/me`: la implementación **correctamente lo rechaza** (no está en `USER_PROFILE_FIELDS`). El PRD legitimaba lo que el código bien hace al negar — un usuario no debe poder auto-desactivar la exigencia de MFA |
| Cabecera | `ADR-016` retirado de "ADRs aplicables" (es el cierre de MOD01; la regla de completitud vive en ADR-022). Añadidos ADR-045 y ADR-086 |

**DT-01 sigue abierto** y sin cambios: la PII no está cifrada en reposo. Ninguna de las dos decisiones lo relaja.
...
- ~~Campo `documentNumber` nunca incluido en respuestas API (minimizacion de datos).~~ **Superado en v1.2.** Se expone en el listado ADMIN (decision CTO 2026-07-23) y en el perfil propio del titular (ADR-086). Fuera de esas dos superficies, la proyeccion minima sigue siendo la regla.
...
- ~~documentNumber nunca se incluye en UserResponseDto (minimizacion de datos).~~ **Superado en v1.2.** `toDto` es un allowlist comun a `findAll` y `findMe`, y ambas superficies tienen finalidad declarada. Ver ADR-086 §2.
...
- PATCH /api/v1/users/me: actualiza campos de perfil propio (firstName, lastName, phone, jobTitle, documentType, documentNumber, avatarUrl). *(v1.2: `mfaRequired` retirado — no esta en `USER_PROFILE_FIELDS` y el codigo lo rechaza; un usuario no debe poder auto-desactivar la exigencia de MFA.)*
...
**Excluidos:** passwordHash, mfaSecret, tokens, failedLoginAttempts, lockedUntil. *(v1.2: `documentNumber` deja de figurar como excluido — se expone con finalidad declarada; ver ADR-086.)*
...
- ~~documentNumber excluido de API responses (minimizacion de datos).~~ **Superado en v1.2.** La minimizacion se cumple por finalidad declarada por campo (mecanismo de ADR-067 §2), no por exclusion total. **Condicion de ADR-086 §4:** una superficie que reciba el campo y no lo renderice no esta cubierta y debe retirarlo.
